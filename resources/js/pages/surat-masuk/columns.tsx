...
export const columns = (onEdit: (surat: Surat) => void): ColumnDef<Surat>[] => [
    ...
    },
    {
        ...
        cell: ({ row }) => (
            <div className="text-sm">
                ...
                {row.original.tanggal_terima && (
                    <p className="text-xs text-muted-foreground">
                        Terima: {formatTanggal(row.original.tanggal_terima)}
                    </p>
                )}
            </div>
        ),
    ...
    {
        accessorKey: 'asal_surat',
        header: 'Asal Surat',
        cell: ({ row }) => (
            <div className="max-w-[220px]">
                <p className="truncate text-sm font-medium">{row.getValue('asal_surat') || '-'}</p>
                {row.original.pengirim && (
                    <p className="truncate text-xs text-muted-foreground">{row.original.pengirim}</p>
                )}
            </div>
        ),
    },
    {
        ...
        cell: ({ row }) => (
            <div className="max-w-[240px]">
                ...
                {row.original.klasifikasi_surat && (
                    <Badge variant="outline" className="mt-1 text-[10px] font-mono">
                        {row.original.klasifikasi_surat.kode}
                    </Badge>
                )}
            </div>
        ),
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem asChild>
                            <Link href={`/surat-masuk/${row.original.id}`}>Detail</Link>
                        </DropdownMenuItem>
